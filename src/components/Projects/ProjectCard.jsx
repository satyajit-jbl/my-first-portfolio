...
const ProjectCard = ({ project }) => {
  ...
  return (
    <>
      <motion.div
        className="flex flex-col items-center gap-8 md:flex-row md:gap-12 w-full max-w-[600px]"
        whileHover={{ y: -5 }}
        transition={{ type: "spring", stiffness: 100 }}
      >
        <img
          src={project.image}
          alt={project.projectsName}
          className="w-full md:w-1/2 cursor-pointer rounded-2xl transition-all duration-300"
        />
        ...
        <div className="flex flex-col gap-5 text-center md:text-left w-full md:w-1/2">
          ...
          <div className="flex flex-wrap gap-2">
            {project.Technologies.slice(0, 4).map((tech, index) => (
              <span
                key={index}
                className="rounded-lg bg-black p-1 text-white text-xs md:text-sm"
              >
                {tech}
              </span>
            ))}
